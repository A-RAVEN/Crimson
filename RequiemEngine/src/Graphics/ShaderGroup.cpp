...
void PipelinePackageResource::BuildPipeline()
{
	...
	{
		PShaderModule p_module = m_OwningDevice->CreateShaderModule(result.second.data(), result.second.size() * sizeof(uint32_t));
	...
	{
		PShaderModule new_module = m_OwningDevice->CreateShaderModule(m_Descriptor.m_Shaders[i].second.data(), m_Descriptor.m_Shaders[i].second.size() * sizeof(uint32_t));
